...
export class XmlRpcRequest {
    ...
    marshal(data) {
        // Vars
        const type = XmlRpc.getDataTag(data);

        const scalar_type = XmlRpc.SCALAR.replace(/\$\{TYPE\}/g, type);
        let xml = "";
        let value;
        let i;
        let member;
...
Date.prototype.toIso8601 = function() {
    ...
    const time = this.toTimeString().substr(0, 8);
...
Date.fromIso8601 = value => {
    ...
    const minute = value.substr(12, 2);
    const sec = value.substr(15, 2);
